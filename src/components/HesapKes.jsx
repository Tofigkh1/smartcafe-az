...
function HesapKes({ orderId, totalAmount }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (selectedPaymentType === 'parca-ode') {
      ...
      paymentData.shares.push({
        ...
        amount: parseFloat(discountedTotal.toFixed(2)), // İndirimli tutarı gönder
        customer_id: selectedCustomerId || null,
      });
    }

    try {
      await axios.post(`${base_url}/order/${orderId.id}/payments`, paymentData, getHeaders());
