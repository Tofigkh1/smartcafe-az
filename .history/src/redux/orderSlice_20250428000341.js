...
export const fetchOrderById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const response = await axios.get(`${base_url}/order/${id}`, getHeaders());
      const data = response.data;
      return {
        orderID: data.id,
        shares: data.shares || [], // API'den gelen shares kullanılmalı
        items: data.items,
      };
    } catch (error) {
...
const orderPersistConfig = {
  key: "order",
  storage,
  whitelist: [], // Boş bırakın veya ihtiyaca göre ayarlayın
};
