...
import { clearOrder, fetchOrderById } from '../redux/orderSlice';
const getHeaders = () => ({
  headers: {
...
useEffect(() => {
  dispatch(clearOrder()); // Önceki veriyi temizle
  dispatch(fetchOrderById(id)); // Yeni veriyi çek
}, [id, dispatch]);
...
useEffect(() => {
  if (id && orderID !== parseInt(id)) { // URL'deki id ile Redux'taki orderID eşleşmiyorsa
