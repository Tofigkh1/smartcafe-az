...
import { clearOrder, setFormattedOrder  } from "../redux/orderSlice";
  ...
  const fetchCustomerInfo = async () => {
    ...
    try {
      ...
      const shareItems = formattedOrder.items.map(item => ({
        name: item.name,
        quantity: item.quantity, // quantity kontrolü
        price: item.price       // price kontrolü
      }));
...
useEffect(() => {
  return () => {
    dispatch(clearOrder()); // Component kapanırken state'i temizle
  };
}, [dispatch]);
