...
function Siparisler() {
  ...
  console.log("orders", orders.map((order) => order.order.id));
  ...
  onClick={() =>
    createObjHesabKes(
      order.order.id, // Əsas sipariş ID'sini göndər
      order.order.total_price,
      order.order.total_prepayment
    )
  }
      ...
      {showDetail && (
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center p-4 z-50">
          <div className="w-full max-w-3xl bg-white rounded-lg shadow-lg overflow-hidden border border-gray-300 relative">
            ...
            <HesapKes quickOrderId={odersIdMassa.id} orderId={odersIdMassa} totalAmount={kalan} />
            </div>
          </div>
        </div>
      )}
