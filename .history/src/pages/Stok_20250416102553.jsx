...
function Stok() {
  ...
  useEffect(() => {
    const fetchRawMaterials = async () => {
      if (detailsItem) { // Detaylar açıldığında çalış
        try {
          const token = localStorage.getItem("token");
          const response = await axios.get(
          ...
          console.log("responseRawMatreals",response);
      ...
      {rawMaterials.length > 0 ? (
        rawMaterials.map((material) => (
          <div key={material.id} className="mt-2">
            ...
            Miqdar: {material.amount} {material.unit || "ədəd"}
          </div>
        ))
      ) : (
        <div className="text-gray-500">Heç bir xammal tapılmadı.</div>
      )}
