...
function Stok() {
  ...
  useEffect(() => {
    const fetchStockAndMaterials = async () => {
      try {
        const formDataToSend = {}; // formDataToSend burada örnek olarak bırakıldı
        ...
        // Ardından raw materials verisi çekiliyor
        ...
        if (
          ...
          console.error("Error adding stock or fetching raw materials:", error);
          alert("Stok eklenirken veya ham maddeler alınırken bir hata oluştu.");
        }
      }
    };
  
    // Eğer bu işlemi bileşen yüklendiğinde yapmak istemiyorsanız, 
    // bir buton veya form submit event’i ile de çağırabilirsiniz.
          ...
          {!addStok ? (
            <>
              <ul className="list-none w-full md:w-1/4">
                ...
                >
                  <i className="fa-solid fa-plus"></i> Grup/Kategori/Menü əlavə
                  edin
                </li>
                <li
                ...
                {groups.map((group) => (
                  ...
                  >
                    <button
                      onClick={() => (
                        setShowPopup(true), setEditGroupid(group.id)
                      )}
                      className="mr-2"
                    >
              ...
              <div className="w-full md:w-3/4">
                <div className="flex flex-col md:flex-row items-center gap-3 mb-3">
                  <p className="my-2">
                    Siyahida toplam {filteredItems.length} qeyd vardir.
                  </p>
                  <button
                      ...
                      {filteredItems.map((item) => (
                        ...
                        >
                          ...
                          <td className="p-3 text-center">
                            {
                              groups.find(
                                (group) => group.id === item.stock_group_id
                              )?.name
                            }
                          </td>
                          <td className="p-3 text-center">
            ...
            <AddStok
              setAddStok={setAddStok}
              item={detailsItem}
              onClose={() => setAddStok(false)}
            />
          )}
        </div>
      </section>
      ...
      {showDetails && detailsItem && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full md:w-1/2 lg:w-1/3">
            ...
            <p>
              <strong>Qr Menü:</strong>{" "}
              {detailsItem.show_on_qr ? "Evet" : "Hayır"}
            </p>
            <p>
              <strong>Grup:</strong>{" "}
              {
                groups.find((group) => group.id === detailsItem.stock_group_id)
                  ?.name
              }
            </p>
